  ...
  try {
    ...
    await Promise.all(
      elections.map(async (election) => {
        ...
        // If the election is marked as 'Pending', also make it active=false it on the blockchain
        if (election.status === "Pending") {
          let toggleStatusResult
          try {
            console.log("called to change status on blockchain to ", election.status);
            toggleStatusResult = await blockchainToggleElectionStatus(election._id.toString(), false);
          }
          catch (error) {
            console.error(
              "Error changing election status on blockchain:",
              toggleStatusResult
            );
          }
        }
  ...
  try {
    ...
    await Promise.all(
      elections.map(async (election) => {
        ...
        // If the election is marked as 'Ongoing', also make it active=true it on the blockchain
        if (election.status === "Ongoing") {
          let toggleStatusResult
          try {
            console.log("called to change status on blockchain to ", election.status);
            toggleStatusResult = await blockchainToggleElectionStatus(election._id.toString(), true);
          }
          catch (error) {
            console.error(
              "Error changing election status on blockchain:",
  ...
  try {
    ...
    await Promise.all(
      elections.map(async (election) => {
        ...
        // If the election is marked as 'Finished', also finish it on the blockchain
